...
class ReactKarma:
    ...
    async def karmaboard(self, ctx: commands.Context, top: int=10):
        ...
        members_sorted = sorted(
            await self._get_all_members(ctx.bot),
    ...
    async def get_karma(self, ctx: commands.Context,
        ...
        await ctx.send("{0} has {1} karma."
                       "".format(user.display_name, karma))
    ...
    async def _interactive_emoji_setup(self, ctx: commands.Context, type_: str):
        msg = await ctx.send("React to my message with the new {} emoji!"
                             "".format(type_))
        try:
        ...
        if isinstance(emoji, discord.Emoji):
            ...
            await ctx.send("Setting the {} failed. This is a custom emoji"
        ...
        await ctx.send("Done! The {} emoji in this server is now {!s}"
                       "".format(type_, emoji))
    ...
    async def reset_karma(self, ctx: commands.Context, user: discord.Member):
        ...
        await ctx.send(
            "{}'s karma has been reset to 0.".format(user.display_name))
    ...
    async def _check_reaction(self,
                              reaction: discord.Reaction,
                              user: discord.User,
                              *,
                              added: bool):
        message = reaction.message
        (author, channel, guild) = (message.author, message.channel,
                                    message.guild)
    ...
    async def _get_all_members(self, bot):
        ...
        for member in bot.get_all_members():
            ...
            if karma == 0:
                continue
            ret.append(
                member_info(id=member.id, name=str(member), karma=karma))
        return ret
